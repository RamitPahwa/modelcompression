...
def prune_resnet18_conv_layer(model, layer_index, filter_index):
	...
	if layer_index == 0:
		...
		next_conv =  list(model.features._modules.items())[4][1][0].conv1
	...
	if layer_index > 0 and layer_index < 5:
		...
		pt=layer_index%2
		if pt==1:
			conv = list(model.features._modules.items())[3+tt][1][kt].conv1
			next_conv =  list(model.features._modules.items())[3+tt][1][kt].conv2
		else:
			if kt==0:
				conv = list(model.features._modules.items())[3+tt][1][kt].conv2
				next_conv =  list(model.features._modules.items())[3+tt][1][kt+1].conv1
			else:
				conv = list(model.features._modules.items())[3+tt][1][kt].conv2
		next_conv =  list(model.features._modules.items())[3+tt+1][1][0].conv1
		downin_conv =  list(model.features._modules.items())[3+tt+1][1][0].downsample[0]
	...
	if layer_index > 4 and layer_index < 9:
		...
		if pt==1:
			conv = list(model.features._modules.items())[3+tt][1][kt].conv1
			next_conv =  list(model.features._modules.items())[3+tt][1][kt].conv2
		#downout_conv =  list(model.features._modules.items())[3+tt][1][0].downsample[0]
		else:
			if kt==0:
				conv = list(model.features._modules.items())[3+tt][1][kt].conv2
				next_conv =  list(model.features._modules.items())[3+tt][1][kt+1].conv1
				downout_conv =  list(model.features._modules.items())[3+tt][1][kt].downsample[0]
			else:
				conv = list(model.features._modules.items())[3+tt][1][kt].conv2
				next_conv =  list(model.features._modules.items())[3+tt+1][1][0].conv1
				downin_conv =  list(model.features._modules.items())[3+tt+1][1][0].downsample[0]
	...
	if layer_index > 12 and layer_index < 17:
		...
		if pt==1:
			...
			if kt==0:				
				...
				conv = list(model.features._modules.items())[3+tt][1][kt].conv2
				#next_conv =  list(model.features._modules.items())[5+1][1][0].conv1
	...
	if not next_conv is None:
		if layer_index ==0:
			features1 = torch.nn.Sequential(*(replace_layers(model.features, i, [layer_index, layer_index],[new_conv, new_conv]) for i, _ in enumerate(model.features)))
			del model.features
			...
			if pt==1:
				...
				model.features._modules.items()[3+tt][1][kt].conv2 = next_new_conv
					...
					#ds = torch.nn.Sequential(
						#*(replace_layers(list(model.features._modules.items())[3+tt][1][0].downsample, i, [0], \
							#[new_down_conv]) for i, _ in enumerate(list(model.features._modules.items())[3+tt][1][0].downsample)))
					#list(model.features._modules.items())[3+tt][1][kt].downsample = ds
					#list(model.features._modules.items())[3+tt][1][kt].downsample[0] = next_downin_conv
			else:
				if kt==0:
					...
					if tt > 1:
						ds = torch.nn.Sequential(
							*(replace_layers(list(model.features._modules.items())[3+tt][1][kt].downsample, i, [0], \
								[new_down_conv]) for i, _ in enumerate(list(model.features._modules.items())[3+tt][1][kt].downsample)))
						list(model.features._modules.items())[3+tt][1][kt].downsample = ds
				else:                                
					model.features._modules.items()[3+tt][1][kt].conv2 = new_conv
					model.features._modules.items()[3+tt+1][kt][0].conv1 = next_new_conv
					ds = torch.nn.Sequential(
						*(replace_layers(list(model.features._modules.items())[3+tt+1][kt][0].downsample, i, [0], \
							[next_downin_conv]) for i, _ in enumerate(list(model.features._modules.items())[3+tt+1][kt][0].downsample)))
					model.features._modules.items()[3+tt+1][kt][0].downsample = ds
					#list(model.features._modules.items())[3+tt+1][kt][0].downsample[0] = next_downin_conv
			
	 	#list(model.features._modules.items())[3+tt+1][kt][0].downsample = ds
		del conv
	else:
		#Prunning the last conv layer. This affects the first linear layer of the classifier.
		list(model.features._modules.items())[3+tt][1][kt].conv2 = new_conv
